 ...
 * @return string
 */
function get_rights_type($type=0){
	$list = C('GRADE_RIGHT_LIST');
	return $list[$type];
}
 ...
 * @return string
 */
function get_maintain_parts_type($type=0){
	$list = C('PARTS_TYPE');
	return $list[$type];
}
 ...
 * @return:
 */
function get_shop_uid($uid)
{
	$mShop = M('Shop');
	$shop = $mShop->where(array('uid'=>$uid))->find();
	return $shop;
}
 ...
 * @return:
 */
function get_provice_city_area($list)
{
    $city = M('City');
    $area = M('CityArea');
    foreach($list as $key=> $v)
    {
        $cityinfo  =$city->where(array('id'=>$v['cityid']))->find();
        $areainfo  =$area->where(array('id'=>$v['areaid']))->find();

        $list[$key]['province'] = $cityinfo['province'];
        $list[$key]['city'] = $cityinfo['city'];
  ...
  //请求时
function create_sign_request($data,$Secret){
  $value=$Secret.$data['Action'].$data['Key'].json_encode($data['Parameters']).$data['SerialNumber'].$data['TimeStamp'].$data['Version'].$Secret;
  $sign=MD5($value);
  return $sign;
}
 ...
 * $map  type:array    1=>"中文"
 */
function change_to_string(&$data,$map) {
  if($data === false || $data === null ){
    return $data;
  }
  $data = (array)$data;
