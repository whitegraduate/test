...
<body>
    ...
    <div class="main-title">
        <h2><?php echo ($fans["nickname"]); ?>-优惠券管理 </h2>
    </div>

    <div class="cf">
        <a class="btn" href="<?php echo U('add_fans_coupon',array('uid'=>$fans['id']));?>" target-form="ids">发放</a>
        <!-- 高级搜索 -->
        ...
        <form class="ids">
            <table>
				...
				<?php if(!empty($list)): if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$menu): $mod = ($i % 2 );++$i;?><tr>
                        ...
                        <?php if($menu["used_time"] == 0): ?><td>未使用</td>
                        <?php else: ?>
                        <td><?php echo (date("Y-m-d",$menu["used_time"])); ?></td><?php endif; ?>
