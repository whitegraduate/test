...
<div class="geren">
 <ul class="list1" id="myTab" role="tabpanel">
       <li class="dropdown li_li">
          <a href="#" class="dropdown-toggle" data-toggle="dropdown">充电订单<b class="caret"></b></a>
        ...
        <?php if(is_array(C("ORDER_STATUS_CHARGE"))): $i = 0; $__LIST__ = C("ORDER_STATUS_CHARGE");if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$type): $mod = ($i % 2 );++$i;?><li class="li_li <?php if(($_GET['s'] != '') and ($_GET['s'] == $key)): ?>active<?php endif; ?>"><a href="<?php echo U('Fans/order_charge',array('s'=>$key));?>" data-toggle="tabpanel"><?php echo ($type); ?></a></li><?php endforeach; endif; else: echo "" ;endif; ?>
  ...
  <div  id="myTabContent" class="tab-content">
		<ul role="tabpanel" class="tab-pane active order" id="home">
			<?php if(isset($lstOrder)): if(is_array($lstOrder)): $i = 0; $__LIST__ = $lstOrder;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$list): $mod = ($i % 2 );++$i;?><li>
				<p>
					订单号：<?php echo ($list["transid"]); ?> <span><?php echo ($list["statusname"]); ?></span>
				</p>
				<p>下单时间：<?php echo (date("Y年m月d日H时i分",$list["create_time"])); ?></p>
				<p style="display: none;">门店：<?php echo ($list["shop"]); ?></p>
				<div class="ann">
					<a
						href="<?php echo U('Fans/order_charge_details',array('oid'=>$list['id']));?>">
						<div class="sc_ann">详情</div>
					</a>
					<?php if(($list["status"] == 2) and ($list["isremark"] == 0)): ?><a href="#"><div
							class="pj_ann" id="<?php echo ($list["id"]); ?>">评价</div></a><?php endif; ?>
							
					<?php if($list["status"] == 0): ?><a
						href="/wxpay/wap.php?s=/Charge/pay/oid/<?php echo ($list['id']); ?>"><div
							class="pj_ann">支付</div></a><?php endif; ?>
					 
		            <?php if(($list["status"] == 1)): ?><a href="<?php echo U('Fans/complain',array('transid'=>$list['transid']));?>"><div class="pj_ann">投诉</div></a><?php endif; ?>
				</div>
			...
			<div class="noorder">
				...
				<a href="<?php echo U('Charge/index');?>">去看看</a>
			</div><?php endif; ?>
		</ul>
	</div>
</div>
...
(function(){
	...
	$(document).ready(function(){
		...
		$(".pj_ann").click(function(){
			...
			top.location.href = "/wap.php/Fans/remark/flag/CD/oid/"+oid+'.html';
		});
	});
	</script>
 <!-- 用于加载js代码 -->
