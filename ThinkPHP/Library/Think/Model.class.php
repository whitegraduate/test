...
class Model {
    ...
    protected function autoValidation($data,$type) {
    	log::write('autoValidation1,');
        if(!empty($this->options['validate'])) {
    	log::write('autoValidation2,');
            $_validate   =   $this->options['validate'];
            unset($this->options['validate']);
        }elseif(!empty($this->_validate)){
    	log::write('autoValidation3,');
            $_validate   =   $this->_validate;
        }
        // 属性验证
    	log::write('autoValidation4,');
        if(isset($_validate)) { // 如果设置了数据自动验证则进行数据验证
            if($this->patchValidate) { // 重置验证错误信息
    	log::write('autoValidation5,');
                $this->error = array();
            }
    	log::write('autoValidation6,');
            foreach($_validate as $key=>$val) {
    	log::write('autoValidation7,');
                // 验证因子定义格式
                // array(field,rule,message,condition,type,when,params)
                // 判断是否需要执行验证
                if(empty($val[5]) || $val[5]== self::MODEL_BOTH || $val[5]== $type ) {
    	log::write('autoValidation8,');
                    if(0==strpos($val[2],'{%') && strpos($val[2],'}'))
                        // 支持提示信息的多语言 使用 {%语言定义} 方式
                        $val[2]  =  L(substr($val[2],2,-1));
                    $val[3]  =  isset($val[3])?$val[3]:self::EXISTS_VALIDATE;
                    $val[4]  =  isset($val[4])?$val[4]:'regex';
                    log::write('autoValidation8.5,');
                    // 判断验证条件
                    switch($val[3]) {
                        case self::MUST_VALIDATE:   // 必须验证 不管表单是否有设置该字段
                            if(false === $this->_validationField($data,$val)) 
                            { 	log::write('autoValidation8.6,');
                                return false;
                            }break;
                        case self::VALUE_VALIDATE:    // 值不为空的时候才验证
                            if('' != trim($data[$val[0]]))
                            {log::write('autoValidation8.7,');
                                if(false === $this->_validationField($data,$val)) 
                                {log::write('autoValidation8.8,');
                                    return false;
                                }
                            }break;
                        default:    // 默认表单存在该字段就验证
                            if(isset($data[$val[0]]))
                            {log::write('autoValidation8.9,');
                                if(false === $this->_validationField($data,$val)) 
                                {   log::write('autoValidation8.95,'); 
                                	return false;
                                }
                    		}
                    }
    ...
    protected function _validationField($data,$val) {
    	log::write('_validationField1,');
        if($this->patchValidate && isset($this->error[$val[0]]))
        {

        	log::write('_validationField2,');
            return ; //当前字段已经有规则验证没有通过
        }log::write('_validationField2.5,');
        if(false === $this->_validationFieldItem($data,$val)){
    		log::write('_validationField3,');
            if($this->patchValidate) {
    			log::write('_validationField4,');
                $this->error[$val[0]]   =   $val[2];
            }else{
    			log::write('_validationField5,');
                $this->error            =   $val[2];
                return false;
            }
        }
        log::write('_validationField6,');
        return ;
    }
    ...
    protected function _validationFieldItem($data,$val) {
    		log::write('_validationFieldItem1,');
    		log::write('_validationFieldItem2='.strtolower(trim($val[4])));
